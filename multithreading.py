...
import numpy as np
import time
# import dlib  # Not needed - using OpenCV tracker instead
...
def countVehicles(param):
	...
	trackers = [] # List of all OpenCV trackers (replaced dlib with OpenCV)
	...
	# Using OpenCV KCF tracker (doesn't require dlib)
		...
		while(cap.isOpened()):
			...
			if skipped_frames_counter == skip_frames:
				...
				for class_index in classes.keys():
					...
					for i in range(len(np_detections[class_index])):
						...
						if np_detections[class_index][i][4] >= confidence_level:
							...
							# Use OpenCV KCF tracker instead of dlib
							tracker = cv2.TrackerKCF_create()
							bbox = (int(startX * width_scale), int(startY * height_scale), 
							        int((endX - startX) * width_scale), int((endY - startY) * height_scale))
				...
				for tracker in trackers:
					...
					if success:
						# Unpack OpenCV bbox format: (x, y, width, height)
						x, y, w, h = [int(v) for v in bbox]
						
						# Scale back to detection coordinates
						startX = int(x / width_scale) if width_scale > 0 else x
						startY = int(y / height_scale) if height_scale > 0 else y
						endX = int((x + w) / width_scale) if width_scale > 0 else (x + w)
						endY = int((y + h) / height_scale) if height_scale > 0 else (y + h)
